...
class PromptBuilder:
    ...
    def _make_a_block(self, retrieved_context):
        ...
        block_str = (
            "\n".join(
                [f_path_comment, f_paths_str, self.seperator]
                + content_lines_comment
                + [self.seperator]
            )
            + "\n"
        )
    ...
    def _make_an_extended_block(self, task_metadata, retrieved_context):
        ...
        duplicate_num = len(
            metadata
        )  # for those share the exact same code fragment from different files
        for i in range(duplicate_num):
            ...
            original_code = Tools.read_code(os.path.join(base_dir, *metadata[i]["fpath_tuple"]))
            ...
            if (
                metadata[i]["fpath_tuple"] == tuple(task_metadata["fpath_tuple"])
                and new_end_line_no >= task_metadata["line_no"]
            ):
                continue
    ...
    def _build_prompt(self, mode, prompt, task_metadata, top_k_context):
        ...
        current_token_length = (
            20  # the length of the head_prompt, same for codex and codegen tokenizer
        )
        prepend_blocks = []
        chosen_context = []
        make_block_func = (
            self._make_an_extended_block if mode == CONSTANTS.rg else self._make_a_block
        )
        for retrieved_context in top_k_context[::-1]:
            ...
            if current_token_length + token_len < self.max_retrieval_length:
                ...
                continue
        prepend_context += "".join(
            prepend_blocks
        )  # all the blocks already have a line break at the end
    ...
    def build_2nd_stage_input_file(self, mode):
        ...
        for query_line in self.query_lines_with_retrieval_results:
            ...
            new_prompt, chosen_context = self._build_prompt(
                mode, old_prompt, task["metadata"], top_k_context
            )
            new_prompt_line = {
                "prompt": new_prompt,
            ...
            new_prompt_line["metadata"]["slice_size"] = chosen_context[0][0]["metadata"][0][
                "slice_size"
            ]
...
class BuildPromptWrapper:
    ...
    def _run(self, mode, query_window_path_builder, output_file_path):
        ...
        for repo in self.repos:
            ...
            retrieval_results = FilePathBuilder.retrieval_results_path(
            ...
            worker = PromptBuilder(
                query_lines_with_retrieval_results, self.task_path, log_message, self.tokenizer
            )
